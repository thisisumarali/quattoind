      ...
      {status.submitted ? (
        <div className="text-center py-8 space-y-4 animate-fade-in">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto leading-relaxed">
            Thank you for reaching out to Quattro Industries. Our technical team will get back to you at the email address provided within 24 hours.
          </p>
          <button
            onClick={() => setStatus((prev) => ({ ...prev, submitted: false }))}
            className="mt-4 px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider text-slate-900 bg-slate-100 hover:bg-slate-200 transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {status.error && (
            <div className="p-4 rounded-xl bg-rose-50 border border-rose-100 text-xs font-semibold text-rose-600">
              {status.error}
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="e.g. RO Membrane Inquiry"
              className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:border-sky-500 focus:ring-4 focus:ring-sky-500/10 text-sm font-medium transition-all"
            />
          </div>
